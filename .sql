...
CREATE VIEW new_employees_details AS  SELECT E.id, Fname, Salary, Hire_date  
    FROM Employees E  WHERE hire_date > DATE '2015-01-01';
...
CREATE TRIGGER BooksDeleteTrigger ON MyBooksDB.Books  
AFTER DELETE AS  
...
BEGIN TRANSACTION  
    ...
    VALUES (123, GETDATE(), CURRENT_USER);  
...
CREATE TABLE Users (
    ...
    PasswordHash VARCHAR(255)  -- Store hashed passwords, not plain text!
);
SQL = "SELECT * FROM Users WHERE username = ? AND password = ?";  
db.execute(SQL, [user, pw]);
SELECT * FROM Users WHERE username = 'somebody' AND password = 'lol';  
